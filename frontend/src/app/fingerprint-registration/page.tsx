  ...
  const handleRegistration = async () => {
    ...
    try {
      const initResponse = await fetch(
        `${API2}/api/scan/init-register?nrcNumber=${user?.nrcNumber}&userId=${user?.id}`,
        {
          credentials: "include",
        },
      );
      ...
      const verifyResponse = await fetch(`${API2}/api/scan/verify-register`, {
        method: "POST",
        credentials: "include",
        headers: {
  ...
  return (
    <div className="bg-gray-100 relative flex h-screen items-center justify-center">
      ...
      <div className="relative z-10 min-h-screen px-4 py-30">
        <HorizontalNav />
        <div className="mt-8 w-80 rounded-lg bg-white p-6 text-center shadow-md">
